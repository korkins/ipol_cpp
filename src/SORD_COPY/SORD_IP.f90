...
!               Surface contribution at all boundaries
                I1B(:NG1, NB) = MU0E0*RM0(:, 1)
                DO IB = 1, NL ! NL=NB-1
                    I1B(:NG1, IB) = I1B(:NG1, IB) + I1B(:NG1, NB)*EUPG(:, NB-IB)
                END DO ! IB = 1, NL
!               Polarized components of the vector surface
                IF (ISRF > 5) THEN ! Polarized reflectance
                    Q1B(:NG1, NB) = MU0E0*RM0(:, 2)
                    U1B(:NG1, NB) = MU0E0*RM0(:, 3)
                    DO IB = 1, NL
                        Q1B(:NG1, IB) = Q1B(:NG1, IB) + Q1B(:NG1, NB)*EUPG(:, NB-IB)
                        U1B(:NG1, IB) = U1B(:NG1, IB) + U1B(:NG1, NB)*EUPG(:, NB-IB)
                    END DO ! IB = 1, NL
                END IF ! ISRF > 5
            END IF ! ISRF > 0
!
...
!===============================================================================
! 06Sep17 - Surface contribution at all boundaries, SRFBNDM_IP, is removed.
!           Reasons: did not get compiled with f2py & small subroutine.
!           This piece of code is now implemented without subroutine.
!
