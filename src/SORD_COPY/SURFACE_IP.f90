...
! LOCAL VARIABLES
    ...
    REAL*8 &
        ...
        SRF    ! Azimuthaly-independent surface coefficient (e.g., Nadal-Breon)
!
! LOCAL ARRAYS
    ...
    REAL*8, DIMENSION(NG1, NR) :: &
        SRFS ! Same as SRF but stores values in RAM to apply symmetry
...
!
            ...
            END DO ! IR = 1, NG1
!
        CASE (8:9) ! Nadal-Breon vegetation & bare soil
            DO IR = 1, NRU
                DO IG = 1, NG1
                    CALL FRESNELR_IP(PSRF(1), PSRF(2), MUI(IG), MUR(IR),    &
                                        AZI, NAZ, BPDF)
                    CALL NADBRESRF(MUI(IG), MUR(IR), MSRF, SRF)
                    R11(:, IG, IR) = SRF*BPDF(:, 1)
                    R21(:, IG, IR) = SRF*BPDF(:, 2)
                    R31(:, IG, IR) = SRF*BPDF(:, 3)
                    R12(:, IG, IR) = SRF*BPDF(:, 4)
                    R22(:, IG, IR) = SRF*BPDF(:, 5)
                    R32(:, IG, IR) = SRF*BPDF(:, 6)
                    R13(:, IG, IR) = SRF*BPDF(:, 7)
                    R23(:, IG, IR) = SRF*BPDF(:, 8)
                    R33(:, IG, IR) = SRF*BPDF(:, 9)          
                END DO ! IG = 1, NG1
            END DO ! IR = 1, NR
!
            DO IR = 1, NG1
                DO IG = 1, NG1
                    CALL FRESNELR_IP(PSRF(1), PSRF(2), MUI(IG), MUR(IR+NRU), &
                                        AZI, NAZ, BPDF)
                    IF (IG < IR) THEN ! IG < IR: do not compute the diagonal
                        SRFS(IG, IR) = SRFS(IR, IG)
                    ELSE ! IG >= IR; Symmetry: diagonal elements ONLY
                        CALL NADBRESRF(MUI(IG), MUR(IR+NRU), MSRF, SRFS(IG, IR))
                        R11(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 1)
                        R22(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 5)
                        R33(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 9)
                    END IF ! IG < IR                 
!
                    R21(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 2)
                    R31(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 3)
                    R12(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 4)
                    R32(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 6)
                    R13(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 7)
                    R23(:, IG, IR+NRU) = SRFS(IG, IR)*BPDF(:, 8)
                END DO ! IG = 1, NG1
            END DO ! IR = 1, NG1
!
