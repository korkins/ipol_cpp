...
!
    SELECT CASE (ISRF)
        ...
        CASE (6:7) ! NT & GC Ocean
            ...
            END DO ! IR = 1, NR
        CASE (8:9) ! Nadal-Breon's vegetation and bare soil models
            DO IR = 1, NR
                CALL FRESNELR0_IP(PSRF(1), PSRF(2), MU0, MUR(IR), AZI, &
                                  NAZ, BPDF)
                CALL NADBRESRF(MU0, MUR(IR), MSRF, SRF)
                R01(:, IR) = SRF*BPDF(:, 1) ! I-component
                R02(:, IR) = SRF*BPDF(:, 2) ! Q-component
                R03(:, IR) = SRF*BPDF(:, 3) ! U-component
            END DO ! IR = 1, NR
